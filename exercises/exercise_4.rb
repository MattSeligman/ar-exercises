...
puts 'Mens Apparel'
for stores in @mensApparel
  puts "Store Name: #{stores.name}\nAnnual Revenue: #{stores.annual_revenue}"
end

@womensApparel = Store.where(womens_apparel: true).and( Store.where('annual_revenue < 1000000') )

puts 'Womens Apparel'
for stores in @womensApparel
  puts "Store Name: #{stores.name}\nAnnual Revenue: #{stores.annual_revenue}"
end